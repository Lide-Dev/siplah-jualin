...
define("TWO_HUNDRED_MILLION", 200000000);
define("FIFTY_MILLION", 50000000);
	...
	function get_product_form_data($product)
	{
		...
		if (!empty($product)) {
			...
			if ($product->listing_type == 'sell_on_site') {
				...
				if (($product->price / 100) > TWO_HUNDRED_MILLION || ($product->price / 100) > FIFTY_MILLION) {
					$data->add_to_cart_url = base_url() . 'compare/compared_product';
					$ci->session->set_userdata("compared_products_id", array($product->id));
				} else {
